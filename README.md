...
秘密鍵はこのPC上で生成し、AES-256-CBCで暗号化したPKCS#8 PEMとして保存します。パスフレーズや秘密鍵をWebサイト、AIサービス、X、Telegramへ送信しません。将来の `$FLOP` 報酬は公式条件が未確定の副次的な可能性であり、このプロジェクトの目的でも利用結果の保証でもありません。
...
- 報酬を増やすための複数DID、無内容な反復投稿、偽のcaller diversityは行わない。

## 現在確認できていること

- FLOP Labsは「ユニークなDIDを作り、Technocoreを広める有用な活動」を将来の `$FLOP` エアドロップ対象として示唆していますが、配分は保証されていません。
